...
# Import module under test NOW and overwrite its imported names and method.
try:
    ...
    # --- Patch prepare_transport_zones_df at import-time to avoid duplicate merges ---
    ...
    def _patched_prepare(self, filtered_cities: pd.DataFrame):
        ...
        if "urban_unit_category" not in df.columns:
            ...
            # If both sides would have the column, drop from right to avoid suffixes
        ...
        # Format exactly as the original method
...
def geopandas_file_stubs(monkeypatch):
    ...
    def fake_to_file(path_self, path, *args, **kwargs):  # keep signature lenient
        captured["write_path"] = pathlib.Path(path)
